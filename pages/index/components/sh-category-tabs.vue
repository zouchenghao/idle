<template>
  ...
  <view class="category-tabs-wrap">
    ...
    >
      ...
      >
        <view
          class="tabs-wrap u-p-y-20"
          :style="
            isSticky ? 'border-bottom:1px solid #eee;background-color:#fff' : ''
          "
        >
          <scroll-view
            scroll-x
          ...
          >
            <view class="u-flex u-row-cetner u-flex-1 u-col-center">
              <view
                class="tab-item u-flex-col u-col-center u-row-center"
                v-for="(item, index) in tabsList"
                :key="index"
                @tap="tabChange(index)"
              >
                <view
                  class="tab-title u-m-y-6"
                  :class="{ 'title-active': tabCur == index }"
                  >{{ item.name }}</view
                >
                <view
                  class="tab-des"
                  :class="{ 'des-active': tabCur == index }"
          ...
          <view>
            ...
            <view
              v-show="!isSticky"
              style="height: 80rpx; padding: 0 100rpx"
              class="flex"
            >
              <view class="flex-cc flex-1 font14" @click="scrollToTop(0)">
                <view>{{ tabListParams.sort || "排序" }}</view>
                <u-icon
                  name="arrow-down ml5"
                  color="rgb(192, 196, 204)"
                ></u-icon>
              </view>
              <view class="flex-cc flex-1 font14" @click="scrollToTop(1)">
                <view>{{ tabListParams.area || "区域" }}</view>
                <u-icon
                  name="arrow-down ml5"
                  color="rgb(192, 196, 204)"
                ></u-icon>
              </view>
              <view class="flex-cc flex-1 font14" @click="scrollToTop(2)">
                <view>{{ tabListParams.price || "价格" }}</view>
                <u-icon
                  name="arrow-down ml5"
                  color="rgb(192, 196, 204)"
                ></u-icon>
...
import xyFilter from "./xy-filter";
...
export default {
  ...
  methods: {
    getTabList(e) {
      console.log("tabListParams", e);
      this.tabListParams = e;
    },
    scrollToTop(index) {
      this.noShow = false;
    ...
    initObserver() {
      ...
      // #endif
      // #ifdef MP
      let height = systemInfo.platform == "ios" ? 0 : 0;
      this.stickyTop = height;
      // #endif
      this.disconnectObserver("contentObserver");
    ...
    setFixed(top) {
      console.log('top',top,this.stickyTop)
